...
def compute_episode_metrics(infos):
    ...
    for step_info in infos:
        for agent_id, data in step_info.items():
            ...
            agent_data[agent_id]["acceleration"].append(float(data.get("acceleration", 0.0)))
            agent_data[agent_id]["route_completion"].append(float(data.get("route_completion", 0.0)))
    ...
    global_metrics = {
        ...
        "avg_acceleration": np.mean([m["avg_acceleration"] for m in agent_metrics.values()]),
        "avg_route_completion": np.mean([m["avg_route_completion"] for m in agent_metrics.values()]),
...
if __name__ == "__main__":
    ...
    metrics = calculate_metrics(epochs = 5,
